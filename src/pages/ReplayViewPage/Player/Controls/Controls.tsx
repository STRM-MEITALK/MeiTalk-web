  ...
  return (
    <STC.ControlsWrapper onMouseOver={onClickScreen} onMouseLeave={onLeaveScreen}>
      ...
      {/* {useCaption && captionLang && (
        <STC.Caption>
          <STC.Gradient />
          {captionObj && captionObj[captionLang] && captionObj[captionLang].length > 0 ? (
            captionObj[captionLang].map((item) => (
              <STC.CaptionText key={`${item.timeStamp}`}>{item.text}</STC.CaptionText>
            ))
          ) : (
            <STC.NoCaptionText>{t('no_caption')}</STC.NoCaptionText>
          )}
        </STC.Caption>
      )} */}
    </STC.ControlsWrapper>
