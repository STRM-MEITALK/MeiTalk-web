...
import { ICommentType } from '@type/commentType';
...
const CommentItem = ({ item }: { item: ICommentType }) => {
  ...
  return (
    <STC.Container>
      <Profile type="ps36" src={item.userPicture} channelId={item.channelId} />
      <STC.CommentBlock>
        <STC.Flex>
          <STC.AccountName>{item.userName}</STC.AccountName>
          <STC.CommentDate>{moment(item.createTime).format('YYYY.MM.DD')}</STC.CommentDate>
        </STC.Flex>
        {/* {isEdit ? (
          <STC.EditBox>
        ...
        <STC.Flex>
          <STC.CommentInfo>
            {/* {!item.deleted && (
              <STC.Flex>
                <STC.CommentImg src={item.likeIs ? LikeImg : NoLikeImg} onClick={onToggleLike} />
                <STC.CntText>{numberWithComma(item.likeCount)}</STC.CntText>
              </STC.Flex>
            )} */}
            <STC.Flex>
              <STC.CommentImg src={ReplyImg} onClick={onHandleShowReply} />
        ...
        {/* {showInput && (
          <STC.ReplyInputContent>
            <STC.ReplyInputBox>
              ...
              />
              <STC.ReplyBtn type="button" value={t('comment_camel_reply').toString()} onClick={() => onEnterReply()} />
            </STC.ReplyInputBox>
            <STC.CancelButton
              type="button"
              value={t('common_cancel').toString()}
              onClick={() => onHandleShowInput(false)}
            />
          </STC.ReplyInputContent>
        )} */}
        {showReply ? (
